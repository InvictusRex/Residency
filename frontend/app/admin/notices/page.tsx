...
export default function AdminNoticesPage() {
  ...
  const pinMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: queryKeys.notices() }),
  })
    ...
    <Shell title="Notice management">
      <div className="page-heading">
        <div>
          ...
          <p className="subheading">Publish official updates to residents. Important notices are pinned.</p>
        </div>
  ...
  return (
    <Dialog open={open} onClose={onClose} title={notice ? 'Edit notice' : 'Create notice'}>
      ...
      <label className="check-row">
        ...
        Important (pinned, residents are notified by email)
      </label>
...
function ConfirmDeleteDialog({
  ...
  return (
    <Dialog open={true} onClose={onClose} title="Delete notice" description={`Delete "${notice.title}"? This cannot be undone.`}>
      {error && <p className="form-error">{error}</p>}
      <div className="dialog-actions">
        <button className="outline" onClick={onClose} disabled={m.isPending}>
          Cancel
        </button>
        <button className="primary" style={{ background: '#e53935', color: '#fff' }} onClick={() => m.mutate()} disabled={m.isPending}>
